            ...
            <table className="mt-6 justify-center mr-2 border-2 border-amber-300">
                <thead className="border">
                    <tr> 
                        ...
                        <th className="px-6 border border-amber-300">Comentarios y Recomendaciones</th>
                    </tr>
                </thead>
                <tbody>
                    {/* Mapeo para los datos de invitaciones */}
                    {data?.map((invitation: Invitation) => (
                        <tr key={invitation.name} className="text-center break-words text-balance font-medium">
                            <td className="px-12 py-2 border border-amber-300">{invitation.name}</td>
                            <td className="px-12 py-2 border border-amber-300">{invitation.assist ? 'Sí' : 'No'}</td>
                            <td className="px-12 py-2 border border-amber-300">{invitation.comments}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            <table className="mt-6 justify-center border-2 border-amber-300">
                ...
                <tbody>
                    ...
                    {data2?.map((theme: Theme) => (
                        <tr key={theme.name} className="text-center break-words text-balance font-medium">
                            <td className="px-12 py-2 border border-amber-300">{theme.name}</td>
                            <td className="px-12 py-2 border border-amber-300">{theme.song}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
